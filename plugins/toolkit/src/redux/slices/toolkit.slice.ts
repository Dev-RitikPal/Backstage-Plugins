...
export const getMyToolkits = createAsyncThunk("toolkit/myToolkits", async (toolkitApi: ToolkitApi) => {
...
export const getToolkits = createAsyncThunk('toolkit/getToolkits', async (toolkitApi: ToolkitApi) => {
...
export const createTool = createAsyncThunk('toolkit/create', async (data: ICreateData, api) => {
...
export const updateTool = createAsyncThunk('toolkit/update', async (data: ICreateData, api) => {
    const response = await data.toolkitApi.updateToolkit(data.body, data.id || 0)
    if (!response.ok) {
        throw await ResponseError.fromResponse(response);
    }
    api.dispatch(getMyToolkits(data.toolkitApi))
...
export const deleteTool = createAsyncThunk("toolkit/delete", async ({ toolkitApi, id }: IDeleteTool, api) => {
...
export const addTool = createAsyncThunk("toolkit/add", async ({ toolkitApi, toolkits }: IAddTool, api) => {
...
export const removeTool = createAsyncThunk("toolkit/remove", async ({ toolkit, toolkitApi }: IRemoveTool, api) => {
...
export const toolkitSlice:Slice<IToolkitState> = createSlice({
    ...
    extraReducers: {
        ...
        [createTool.fulfilled.type || updateTool.fulfilled.type]: (state, action) => {
            state.create.loading = false;
            state.create.error = '';

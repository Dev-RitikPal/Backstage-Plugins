...
export const createUpdateToolkit = async (req: any, res: Response) => {
    try {
        ...
        if (req.params.id) {
            ...
            if (toolkit) {
                if (type === 'private') {
                    await client('toolkit_item').where({ toolkit: req.params.id }).whereNot({ user: user.identity.userEntityRef }).delete()
                }
...
export const addToolkit = async (req: any, res: Response) => {
    try {
        ...
        const previousData = await client('toolkit_item').where({ user }).select('toolkit')
        const checkIsExists = await client('toolkit').whereIn('id', toolkits).where({ type: 'public' })
...
export const getToolkits = async (req: any, res: Response) => {
    try {
         ...
         */
        let toolOfUser = await client('toolkit_item').where({ 'toolkit_item.user': user }).select('toolkit')
        toolOfUser = toolOfUser.map(({ toolkit }) => toolkit)

        const data = await client('toolkit_item').whereNot({ 'toolkit_item.user': user, 'toolkit.type': 'private' }).whereNotIn('toolkit.id', toolOfUser).innerJoin('toolkit', 'toolkit.id', 'toolkit_item.toolkit')
        res.status(200).json(data)
    }
    catch (error) {
        console.log("🚀 ~ file: toolkit.controller.ts:31 ~ addTookit= ~ error", error)
...
export const deleteToolkitById = async (req: any, res: Response) => {
    try {
        ...
        const data = await client('toolkit').where({ id, owner: user.identity.userEntityRef }).delete()
        if (data) {
